...
const MenuModal = ({ dish, onClose }) => {
  ...
  const handleAddToCart = async () => {
    ...
    if (existingItem) {
      // Already in cart → Update Quantity (add the selected quantity to existing)
      await updateQuantity(dish.DishID, existingItem.quantity + quantity);
    } else {
      // Not in cart → Add to Cart with selected quantity
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex justify-center items-center p-3 sm:p-4 backdrop-blur-sm overflow-y-auto">
      <motion.div
        ...
        className="relative bg-white rounded-xl sm:rounded-2xl w-full max-w-sm md:max-w-5xl max-h-[80vh] md:max-h-[80vh] flex flex-col shadow-2xl overflow-hidden my-auto"
      >
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col md:flex-row">
          <div className="flex flex-col md:w-1/2">
            {/* Image */}
            <div className="overflow-hidden flex-shrink-0 relative h-40 sm:h-48 md:h-56">
              <img
                src={dish.Image}
                alt={dish.Name}
            ...
            {/* Scrollable Content */}
            <div className="p-4 sm:p-6 space-y-4 sm:space-y-5  flex-1 custom-scrollbar">
              {/* Dish Info */}
              <div>
              ...
              <div className="bg-gray-50 hidden md:block p-3 sm:p-4 rounded-lg sm:rounded-xl border border-gray-100">
                ...
                <div className="flex flex-row sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                  <div className="flex items-center">
                    <button
                      ...
                      className="w-6 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-white rounded-l-lg border border-gray-200 hover:bg-gray-100 transition"
                    >
                      <FaMinus size={10} className="sm:text-xs" />
                    </button>
                    <div className="w-8 h-8 sm:w-12 sm:h-10 flex items-center justify-center border-t border-b border-gray-200 bg-white font-medium text-sm sm:text-base">
                      {quantity}
                    </div>
                    <button
                      onClick={incrementQuantity}
                      className="w-6 h-8 sm:w-10 sm:h-10 flex items-center justify-center bg-white rounded-r-lg border border-gray-200 hover:bg-gray-100 transition"
                    >
                  ...
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className={`w-full sm:flex-1 py-2 sm:py-3 px-2 sm:px-4 rounded-md sm:rounded-lg text-sm sm:text-base font-medium flex items-center justify-center gap-2 shadow-md transition-all ${
                      addedToCart
                  ...
                  >
                    ...
                    <span className="truncate">
                      {addedToCart
                        ? "Added to Cart!"
                        : `Add - ₹${(dish.Price * quantity).toFixed(2)}`}
                    </span>
                  </motion.button>
                </div>
              </div>
              ...
              {/* Recent Reviews */}
              <div className="flex justify-between overflow-y-auto  md:hidden items-center mb-2 sm:mb-3">
              ...
              {reviews.length > 0 ? (
                <div className="space-y-2 md:hidden sm:space-y-3 flex-1 max-h-[30vh] overflow-y-auto pr-2 custom-scrollbar">
                ...
                <div className="bg-gray-50 p-3 sm:p-4 rounded-md md:hidden sm:rounded-lg border border-gray-100 text-center">
          ...
          {/* Desktop Reviews */}
          <div className="w-full md:w-1/2 hidden md:flex md:flex-col p-6">
              ...
              <div className="flex items-center gap-2">
                {user && (
                  <button
                    ...
                    className="text-sm bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded-md font-medium transition-colors"
                  >
                    {getExistingReview(dish.DishID)
                      ? "Edit Review"
                      : "Add Review"}
            ...
            {reviews.length > 0 ? (
              <div className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar max-h-[calc(75vh-8rem)]">
                {(showAllReviews ? reviews : reviews.slice(0, 5)).map((rev) => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      key={rev.ReviewID}
                      className="border p-2 md:p-3 rounded-md sm:rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow"
                    >
                      <div className="flex flex-col xs:flex-row xs:justify-between xs:items-start gap-1 mb-1 sm:mb-2">
                        <div className="flex justify-between gap-1">
                          <div className="flex items-center gap-2">
                            <div className="h-6 w-6 bg-red-500/60 border border-red-500 rounded-full flex items-center justify-center">
                              <span className="text-white text-xs sm:text-sm">
                                {rev.CustomerName.charAt(0)}
                              </span>
                            </div>
                            <div className="text-xs flex flex-col sm:text-sm text-gray-500 mt-1 sm:mt-2 font-medium">
                              {rev.CustomerName}
                              <span className="text-[10px] sm:text-xs text-gray-500">
                                {reviewDate}
                              </span>
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100 text-center flex-shrink-0">
